...
let js_float_of_string = fun s ->
  ...
  with _ -> (
    ...
    with _ -> raise (Failure("js_float_of_string"))
  )
...
and str_expr (e : expr_t) = match e with
  ...
  | CallExpr(_,(ValExpr(_,ClosureVal(_)) as e1),e2) -> Printf.sprintf "(%s)(%s)" (str_expr e1) (str_x_list str_expr e2 ", ")
...
and eq_expr (e1 : expr_t) (e2 : expr_t) = match (e1,e2) with
  ...
  | (BopExpr(_,ea1,bop1,eb1),BopExpr(_,ea2,bop2,eb2)) -> (eq_expr ea1 ea2) && (eq_bop bop1 bop2) && (eq_expr eb1 eb2)
...
| StrVal(s) -> 
  ...
  else (try js_float_of_string s with _ -> Float.nan)
| UndefVal -> Float.nan
